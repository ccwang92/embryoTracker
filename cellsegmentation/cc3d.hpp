    ...
    for (int64_t z = 0; z < sz; z++) {
      for (int64_t y = 0; y < sy; y++, row++) {
        ...
        for (int64_t x = xstart; x < xend; x++) {
          ...
          if (z > 0 && cur == in_labels[loc + E]) {
            out_labels[loc] = out_labels[loc + E];
            if (x > 0 && y > 0 && cur == in_labels[loc + J] && cur != in_labels[loc + A]) {
              equivalences.unify(out_labels[loc], out_labels[loc + J]);
              if (x < sx - 1 && y > 0 && cur == in_labels[loc + L] && cur != in_labels[loc + C]){
                  equivalences.unify(out_labels[loc], out_labels[loc + L]);
              }
            }
            else if (x < sx - 1 && y > 0 && cur == in_labels[loc + L] && cur != in_labels[loc + C]) {
              equivalences.unify(out_labels[loc], out_labels[loc + L]);
              if (x > 0 && cur == in_labels[loc + M] && cur != in_labels[loc + D]) {
                  equivalences.unify(out_labels[loc], out_labels[loc + M]);
              }
            }
            else if(x > 0 && cur == in_labels[loc + M] && cur != in_labels[loc + D]){
                equivalences.unify(out_labels[loc], out_labels[loc + M]);
            }
            else if(y > 0 && cur == in_labels[loc + K] && cur != in_labels[loc + B]){
                equivalences.unify(out_labels[loc], out_labels[loc + K]);
            }
          }
          // It's the original 2D problem now
...
OUT* connected_components3d(
  ...
  if (connectivity == 26) {
    ...
    return connected_components3d_10<T, OUT>(
      in_labels, sx, sy, sz,
      max_labels, out_labels, N
    );
  }
    ...
    throw std::runtime_error("Only 4 and 8 2D and 6, 10, 18, and 26 3D connectivities are supported.");
  }
}
