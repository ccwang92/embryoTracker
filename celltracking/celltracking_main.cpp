...
bool cellTrackingMain::loadTrackResults(vector<int> data_size_yxzt, const QStringList &fileNames){
    ...
    p4tracking.observationCost = -(p4tracking.c_en+p4tracking.c_ex);
...
/**
 * @brief endpoint2loc: identify user's exact annotation location by selecting the max-intensity voxel given the
 * line segment that penetrates the volume with two end points. This point represents the cell user selected.
 * @param nearEnd
 * @param farEnd
 * @return
 */
size_t cellTrackingMain::endpoint2loc(QVector3D nearEnd, QVector3D farEnd, const cellSegmentMain &cellSegment, int t){
    long sz_single_frame = cellSegment.data_rows_cols_slices[0] *
                            cellSegment.data_rows_cols_slices[1] *
                                cellSegment.data_rows_cols_slices[2];
    ...
    traceExtract({nearEnd.y(), nearEnd.x(), nearEnd.z()}, {farEnd.y(), farEnd.x(), farEnd.z()},
                 cellSegment.data_rows_cols_slices, line_width, voxel_idx_in_line_segment);
    size_t max_intensity_voxel_idx;
    unsigned char max_intensity = 0;
    for(size_t v : voxel_idx_in_line_segment){
        if(single_frame->at<unsigned char>(v) > max_intensity){
            max_intensity = single_frame->at<unsigned char>(v);
...
/**
 * @brief loc2traceId:given an annotation (voxel index), return its corresponded trace id;
 * @param idx
 * @param t
 * @return
 */
...
/**
 * @brief loc2traceId:given an annotation (voxel index), return its corresponded node id;
 * @param idx
 * @param t
 * @return
 */
...
/**
 * @brief updateTraceWithOneAnnotation: if user give us an annotation and she/he believes it belongs to an existing trace,
 * we will update our tracking results based on this annotation.
 * @param trace_id
 * @param idx
 * @param cellSegment
 * @param t
...
void cellTrackingMain::updateTraceWithOneAnnotation(int trace_id, size_t idx,
    ...
    if (org_node_id < movieInfo.nodes.size()){
        ...
        org_loc_in_trace = movieInfo.nodes[org_node_id].nodeLocInTrack;
    }
    ...
    /** * step 3. update tracking results **/
    if(org_node_id >= 0){
        if(org_trace_id >= 0){
            if(org_trace_id == trace_id){

            }else{

            }
        }else{
            if (movieInfo.frames[*movieInfo.tracks[trace_id].rbegin()] < t ){
                movieInfo.tracks[trace_id].push_back(org_node_id);
            }else{
                size_t insert_idx = 0;
                // can use lower_bound or binary_search
                FOREACH_i(movieInfo.tracks[trace_id]){
                    if (movieInfo.frames[movieInfo.tracks[trace_id][i]] >= t){
                        insert_idx = i;
                        break;
                    }
                }
                if(movieInfo.frames[movieInfo.tracks[trace_id][insert_idx]] ==  t){
                    auto start = movieInfo.tracks[trace_id].begin() + insert_idx;
                    vector<size_t> new_track(start, movieInfo.tracks[trace_id].end());
                    movieInfo.tracks[trace_id].resize(insert_idx+1);
                    *movieInfo.tracks[trace_id].rbegin() = org_node_id;
                    movieInfo.nodes[org_node_id].nodeId2trackId = trace_id;
                    movieInfo.nodes[org_node_id].nodeLocInTrack = insert_idx;
                    if(new_track.size() > 0){
                        movieInfo.tracks.emplace_back(new_track);
                        for(int i=0; i<new_track.size(); i++){
                            movieInfo.nodes[new_track[i]].nodeId2trackId = movieInfo.tracks.size()-1;
                            movieInfo.nodes[new_track[i]].nodeLocInTrack = movieInfo.tracks.size()-1;
                        }
                    }
                }else{
                    movieInfo.tracks[trace_id].insert(movieInfo.tracks[trace_id].begin()+insert_idx, org_node_id);
                    movieInfo.nodes[org_node_id].nodeId2trackId = trace_id;
                    movieInfo.nodes[org_node_id].nodeLocInTrack = insert_idx;
                }
            }
        }
    }else{ /// this non-detected cell is too complicated and may leave for future works
        //        newFoundCellInfo sNi;
        //        sNi.frame = t;
        //        sNi.threshold = 255;
        //        sNi.voxIdx.push_back(idx);
        //        size_t new_idx = cellSegment.number_cells[sNi.frame] + 1;
...
/**
 * @brief extendTraceWithOneAnnotation: if user give us an annotation and she/he believes it belongs to an existing trace,
 * we add this annotation to the existing trace. Here the existing trace has ended, so we call it trace extension. This is
 * a simplified condition because otherwise we need to consider segmentation update.
 * @param trace_id
 * @param idx
 * @param cellSegment
 * @param t
...
void cellTrackingMain::mccTracker_one2one(bool get_jumpCost_only, bool get_res_from_txt){
    ...
    for(size_t i=0; i < movieInfo.nodes.size(); i++){
        ...
        mcost[arc_cnt] = round((-((double)node.in_cost+(double)node.out_cost + 0.00001)  * 1e7));// make sure all detections are included
        arc_cnt ++;
        // out arc
...
void cellTrackingMain::mccTracker_splitMerge(vector<splitMergeNodeInfo> &split_merge_node_info){
    ...
    for(nodeInfo node : movieInfo.nodes){
        ...
        mtail[arc_cnt] = 2 * node.node_id+1;
        mhead[arc_cnt] =  2 * node.node_id + 2;
        mcost[arc_cnt] = round((double)(p4tracking.observationCost + 0.00001) * 1e7); // add eps to avoid single-detection traces
        arc_cnt ++;
        // out arc
        mhead[arc_cnt] = src_id;
        mtail[arc_cnt] =  2 * node.node_id+2;
